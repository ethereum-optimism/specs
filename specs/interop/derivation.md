...
On the first block at or after the interop  [activation block](#activation-block), where the current chain is in a dependency set with
two or more active chains, a deposit transaction based upgrade transaction is deterministically generated by the
derivation pipeline:
...
Initializing the CrossL2InboxProxy requires `50_000` in gas. This fits within the standard SystemTxMaxGas of `1_000_000`
even allowing for the gas used by the 
