...
Note: when CREATE2 targets an address that already has code, the [zero address is placed on the stack](https://github.com/ethereum/execution-specs/blob/4ef381a0f75c96b52da635653ab580e731d3882a/src/ethereum/forks/prague/vm/instructions/system.py#L112) (execution specs).
...
Conditionally deploys a contract using CREATE2 through the [Deterministic Deployment Proxy](#deterministic-deployment-proxy),
succeeding silently if the contract already exists at the target address.

**Parameters:**

- `_salt`: bytes32 salt value used for CREATE2 address derivation
- `_initCode`: Contract creation bytecode (constructor code + constructor arguments)

**Returns:**

- `deployedAddress`: The address where the contract is or will be deployed

**Behavior:**

- MUST compute the target deployment address
- MUST check if a contract exists at the computed address
- If a contract exists at the target address:
  - MUST return the target address without making any external calls
  - MUST NOT modify any state
  - MUST NOT revert
- If no contract exists at the target address:
  - MUST forward `_salt` and `_initCode` to the [Deterministic Deployment Proxy](#deterministic-deployment-proxy)
    by calling the proxy with calldata `_salt ++ _initCode`
  - MUST return the address returned by the [Deterministic Deployment Proxy](#deterministic-deployment-proxy)
  - MUST propagate any revert from the [Deterministic Deployment Proxy](#deterministic-deployment-proxy)
- MUST return the same address value regardless of whether deployment occurred or was skipped due to collision
- MUST be callable by any address (no access control restrictions)
...
Executes a batch upgrade of multiple [predeploy](#predeploy) contracts by delegating to an [L2ContractsManager](#l2contractsmanager).

**Parameters:**

- `_l2ContractsManager`: Address of the L2ContractsManager contract containing the upgrade logic

**Behavior:**

- MUST revert if the caller is not the [Depositor Account](#depositor-account)
- MUST execute a DELEGATECALL to `_l2ContractsManager` with calldata encoding a call to the `upgrade()` function
- MUST revert if the DELEGATECALL returns failure (returndata indicates revert)
- MUST preserve all ProxyAdmin storage and state during the DELEGATECALL
- MUST emit an appropriate event indicating the upgrade was executed
- MUST NOT modify the owner or other administrative settings of the ProxyAdmin
- MUST be non-reentrant to prevent recursive calls during upgrade execution

**Notes:**

- The function does not accept additional parameters because upgrade logic and configuration are encapsulated in the
  L2ContractsManager contract
- The L2ContractsManager address is provided as a parameter rather than stored to enable different upgrade strategies
  over time
...
chain-specific feature flags. These values are typically stored in system predeploys like [L1Block](#l1block) and must
...
The main entry point called via DELEGATECALL from the [L2ProxyAdmin](#l2proxyadmin). Orchestrates the complete upgrade
process by gathering configuration, computing implementation addresses, and executing all predeploy upgrades.

**Parameters:**

None. All upgrade logic and target addresses are encoded in the contract code itself.

**Behavior:**

- MUST execute deterministically with no dependency on runtime parameters or variable external state
- MUST gather all required [network-specific configuration](#network-specific-configuration) from existing predeploys
- MUST validate that gathered configuration values are within expected ranges and formats
- MUST read [feature flags](#feature-flag) if upgrade behavior is conditional on development/test features
- For each predeploy being upgraded:
  - MUST compute or reference the correct implementation address
  - MUST prepare [initialization parameters](#initialization-parameters) if required by the new implementation
  - MUST call the predeploy proxy's `upgradeTo()` function if no initialization is needed
  - MUST call the predeploy proxy's `upgradeToAndCall()` function with initialization calldata if initialization is needed
  - MUST verify that the upgrade succeeded (proxy's implementation address changed to expected value)
- MUST revert if any predeploy upgrade fails
- MUST revert if configuration gathering fails or returns invalid values
- MUST complete all upgrades within the available gas limits
- MUST NOT modify any storage slots belonging to the [L2ProxyAdmin](#l2proxyadmin)
- MUST emit events for significant upgrade operations (optional but recommended for observability)

**Notes:**

- The function is called in the context of the ProxyAdmin due to DELEGATECALL, giving it the authority to upgrade
  predeploy proxies
- Because the function accepts no parameters, the specific upgrade logic (which predeploys, which implementations) must
  be compiled into the contract code
- Each fork upgrade requires a new L2ContractsManager contract with updated upgrade logic
- Configuration preservation follows the pattern: read from old implementation → upgrade implementation → write to new
  implementation
