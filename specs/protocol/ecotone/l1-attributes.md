...
https://eips.ethereum.org/EIPS/eip-4844#base-fee-per-blob-gas-update-rule.
The `BLOB_BASE_FEE_UPDATE_FRACTION` parameter in the formula varies
according to which L1 fork is active
at the origin block (see e.g. https://eips.ethereum.org/EIPS/eip-7691). It is therefore
necessary for L2 consensus layer clients to know the blob parameters and activation
time for each L1 fork to compute the `blobBaseFee` correctly. 
