...
2. **Bundle Canonicalization**: Prior to cutting a contracts release, the `current-l2-upgrade-bundle.json` should
   ...
   However the current semver convention increments the major version everytime we deploy a new OPCM in order to
...
3. **Client Integration**: The canonical bundle is integrated into L2 client implementations and released with fork
   activation logic. TBD how the client should actually consumre the canonical bundle. Does the contracts package
